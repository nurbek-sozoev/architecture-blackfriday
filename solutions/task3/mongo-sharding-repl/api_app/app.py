...
async def get_shards_members(base_client: motor.motor_asyncio.AsyncIOMotorClient) -> Dict[str, Any]:
    shards_members: Dict[str, Any] = {}
    shards_list = await base_client.admin.command("listShards")
    for shard in shards_list.get("shards", []):
        shard_id = shard["_id"]
        host = shard["host"]
        if "/" in host:
            rs_name, hosts = host.split("/", 1)
        ...
        try:
            await shard_client.admin.command("ping")
            shards_members[shard_id] = {
                "primary": shard_client.primary,
                "secondaries": list(shard_client.secondaries or []),
            }
        finally:
            shard_client.close()
    return shards_members
...
async def root():
    ...
    if topology_type == "Sharded":
        ...
        shards_members = await get_shards_members(client)
